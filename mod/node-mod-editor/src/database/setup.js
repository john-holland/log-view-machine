...
const schema = {
  ...
  // Pending sign-up activations (magic link)
  pendingActivations: `
...
export async function setupDatabase() {
  try {
    ...
    // SaaS indexes
    await db.raw('CREATE INDEX IF NOT EXISTS idx_state_machines_tenant_id ON state_machines(tenant_id)');
    await db.raw('CREATE INDEX IF NOT EXISTS idx_users_tenant_id ON users(tenant_id)');
    await db.raw('CREATE INDEX IF NOT EXISTS idx_pending_activations_tenant_id ON pending_activations(tenant_id)');
...
export const dbUtils = {
  ...
  // Pending activations (sign-up magic link)
  async createPendingActivation(email, name, token, expiresAt) {
    await db('pending_activations').where({ email }).del();
    return await db('pending_activations').insert({
      email,
      name: name || null,
      token,
  ...
  async getPendingActivationByToken(token) {
    return await db('pending_activations').where({ token }).first();
  },

  async deletePendingActivation(token) {
    return await db('pending_activations').where({ token }).del();
  },

  async deletePendingActivationByEmail(email) {
    return await db('pending_activations').where({ email }).del();
  },
