...
**Connection refused to dotCMS?** The editor can run without dotCMS (login uses a stub; preview uses the main server). To use full dotCMS features (templates, persistence), stand up dotCMS: from `example/node-example` run `docker-compose up -d dotcms` (or from this directory `docker-compose up -d dotcms`). Wait 1–2 minutes for dotCMS to be ready on port 8080.
