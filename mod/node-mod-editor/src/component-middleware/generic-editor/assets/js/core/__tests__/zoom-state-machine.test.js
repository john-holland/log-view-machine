...
describe('ZoomStateMachine', () => {
    ...
    describe('Force State Method', () => {
        test('should allow forcing to valid state', () => {
            const result = stateMachine.forceState('canvas_dragging');
            expect(result).toBe(true);
            expect(stateMachine.getCurrentState()).toBe('canvas_dragging');
        });

        test('should reject forcing to invalid state', () => {
            const result = stateMachine.forceState('invalid_state');
            expect(result).toBe(false);
            expect(stateMachine.getCurrentState()).toBe('idle');
        });

        test('should log forced state changes', () => {
            const consoleSpy = jest.spyOn(console, 'log');
            stateMachine.forceState('canvas_hover');
            
            expect(consoleSpy).toHaveBeenCalledWith('Zoom State Machine: idle -> canvas_hover (FORCED)');
    ...
    describe('Reset Method', () => {
        ...
        test('should update indicators after reset', () => {
            stateMachine.transition('MOUSE_ENTER_CANVAS');
            stateMachine.reset();
