...
- **Editor auth and security adapter**: **TomeConfig.auth** can include **requiredForPublish** and **requireEmail** so the editor gates Publish on login and optionally verified email. The client uses a **security adapter** (e.g. **window.securityAdapter**): **getToken(): Promise\<string | null\>** and **getUser(): Promise\<{ id, email? } | null\>**. The editor calls the adapter before **POST /api/editor/publish** and sends **Authorization: Bearer \<token\>** when present. The server (node-example) validates the token when **PUBLISH_REQUIRES_LOGIN** is set (e.g. Google ID token via **google-auth-library**). Implementations: **Google login adapter** (OAuth2 / Google Identity Services, id_token stored and sent); **optional email send adapter** (interface **sendEmail({ to, subject, html })** for magic link or verification, stub in node-example).
