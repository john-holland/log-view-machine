...
/** Route table entry: method + path -> tomeId and machineKey */
 ...
 */
function buildRouteTable(tomeConfigs: TomeConfig[]): Map<string, RouteEntry> {
  ...
  for (const config of tomeConfigs) {
    ...
    for (const [machineKey, binding] of Object.entries(routes)) {
      const b = binding as { path: string; method?: string };
 ...
 */
function eventToNormalizedRequest(event: LambdaHttpEvent): NormalizedRequest {
 ...
 */
function normalizedResponseToLambda(nr: NormalizedResponse): LambdaHttpResponse {
  const headers: Record<string, string> = {
...
let cachedTomeManager: TomeManager | null = null;
let cachedRouteTable: Map<string, RouteEntry> | null = null;
let cachedHandler: (event: LambdaHttpEvent) => Promise<LambdaHttpResponse> = async () => ({ statusCode: 503, body: JSON.stringify({ error: 'Adapter not applied' }) });
 ...
 */
export function createLambdaCaveAdapter(_options?: LambdaCaveAdapterOptions): CaveServerAdapter & { getLambdaHandler(): (event: LambdaHttpEvent) => Promise<LambdaHttpResponse> } {
  let lambdaHandler: (event: LambdaHttpEvent) => Promise<LambdaHttpResponse> = async () => ({ statusCode: 503, body: JSON.stringify({ error: 'Not applied' }) });

  const adapter: CaveServerAdapter & { getLambdaHandler(): (event: LambdaHttpEvent) => Promise<LambdaHttpResponse> } = {
    async apply(context: CaveServerContext): Promise<void> {
      ...
      lambdaHandler = async (event: LambdaHttpEvent): Promise<LambdaHttpResponse> => {
