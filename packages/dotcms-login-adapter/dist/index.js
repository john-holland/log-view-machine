 ...
 */
export function evaluatePermission(user, spec, levelOrder = ['anonymous', 'user', 'admin']) {
...
function toCaveUser(raw, permissionLevel, tenantId) {
    return {
        id: raw.id,
 ...
 */
export function createDotcmsLoginAdapter(options = {}) {
    const { loginHandler, levelOrder = DEFAULT_LEVEL_ORDER, loggedInLevel = 'user', getSessionUser, getTenantName, } = options;
    return {
        async login(credentials) {
            ...
            if (!u?.id)
                return null;
            return toCaveUser({ id: u.id, username: u.username, email: u.email }, loggedInLevel);
        },
 ...
 */
export function deriveTenantFromRequest(req) {
    if (!req || typeof req !== 'object')
        return 'default';
    const r = req;
    const host = typeof r.get === 'function' ? r.get('host') : r.headers?.['host'];
    const protocol = typeof r.protocol === 'string' ? r.protocol : 'http';
    const baseUrl = typeof r.baseUrl === 'string' ? r.baseUrl : (typeof r.url === 'string' ? r.url.split('?')[0] : '');
    if (host) {
